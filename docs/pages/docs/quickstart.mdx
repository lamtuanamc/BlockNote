...
```console
npm install blocknote-core-ts blocknote-react-ts blocknote-mantine-ts
```
...
We also import `blocknote-mantine-ts/style.css` to add default styling for the editor and the `Inter` font that BlockNote exports (optional).
